...
class AutoReply():
    ...
    def auto_reply_to_friend(dialog_window:WindowSpecification,duration:str,callback:Callable[[str,list[str]],str],contexts:list[str]=[],save_file:bool=False,save_media:bool=False,target_folder:str=None,close_dialog_window:bool=True)->dict:
        '''
        该方法通过轮询的方式在指定时间内自动回复会话窗口内的新消息并监听内容
        Args:
...
class Call():
    ...
    def voice_call(friend:str,wait:bool=False,is_maximize:bool=None,close_weixin:bool=None)->(WindowSpecification|None):
        ...
        old_version=version.parse('4.1.7')#4.1.8开始微信把语言和视频电话合并在顶部，并且在简体中文时更名为语音通话
        ...
        if not is_group:
            if current_version>old_version:
                ...
                voice_call_button=main_window.child_window(**Buttons.VoiceCallButton)
                voice_call_button.click_input()
            hwnd=win32gui.FindWindow('Qt51514QWindowIcon',Special_Labels.VoipCall)
            VoipCall_window=Tools.move_window_to_center(Window_handle=hwnd)
            if close_weixin:main_window.close()
            if wait:
                try:
                    #等待出现接通后顶部的计时文本
                    VoipCall_window.child_window(class_name="mmui::VOIPDurationTextView").wait(wait_for='ready',timeout=55)
...
class Collections():
    ...
    def take_notes(text:str,files:list[str]=[],share_moments:bool=False,is_maximize:bool=None,close_weixin:bool=None)->None:
        ...
        if text:edit_area.set_text(text)
        if files:
            SystemSettings.copy_files_to_clipboard(files)
            pyautogui.hotkey('ctrl','v')
        pyautogui.hotkey('ctrl','s')
        if share_moments:
            note_window.child_window(**Buttons.MoreButton).click_input()
...
class Contacts():
    ...
    def get_groupMembers_info(group:str,is_maximize:bool=None,search_pages:int=None,close_weixin:bool=None)->list[str]:
        ...
        chat_history_window=Navigator.open_chat_history(friend=group,is_maximize=is_maximize,search_pages=search_pages,close_weixin=close_weixin)
        if is_group_chat:
    ...
    @staticmethod
    def get_friends_info(is_maximize:bool=None,close_weixin:bool=None)->list[str]:
        '''
        该方法用来快速获取通讯录内好友备注名称
        Args:
    ...
    @staticmethod
    def get_wecom_friends_info(is_maximize:bool=None,close_weixin:bool=None)->list[str]:
        '''
        该方法用来快速获取通讯录内企业微信好友备注名称
        Args:
    ...
    @staticmethod
    def get_serAcc_info(is_maximize:bool=None,close_weixin:bool=None)->list[str]:
        '''
        该方法用来快速获取通讯录内所有服务号名称
        Args:
    ...
    @staticmethod
    def get_offAcc_info(is_maximize:bool=None,close_weixin:bool=None)->list[str]:
        '''
        该方法用来快速获取通讯录内所有公众号名称
        Args:
    ...
    def get_serAcc_detail(interval:float=0.1,is_maximize:bool=None,close_weixin:bool=None,is_json:bool=False)->(list[dict]|str):
        ...
        def get_specific_info():
            ...
            if friendProfile_label in texts:
                description=texts[texts.index(friendProfile_label)+1]  
            else: 
                description='无'
        ...
        if close_weixin is None:
            close_weixin=GlobalConfig.close_weixin
        wxnum_label=Special_Labels.WxNum
        friendProfile_label=Special_Labels.FriendProfile
        friends_detail=[]
        #通讯录列表
        contact_list,main_window=Navigator.open_contacts(is_maximize=is_maximize)
        #右侧的自定义面板
    ...
    def get_groups_info(is_maximize:bool=None,close_weixin:bool=None)->list[str]:
        '''
        该方法用来获取我加入的所有群聊,原理是搜索个人昵称在群聊结果一栏中遍历查找
        Args:
    ...
    def get_friend_profile(friend:str,search_pages:int=None,is_maximize:bool=None,close_weixin:bool=None):
        ...
        '标签':tag,'描述':descrption,'朋友权限':privacy,'共同群聊':f'{common_group_num}','个性签名':signature,'来源':source}
        main_window.child_window(**Buttons.ChatInfoButton).click_input()#点一下聊天信息按钮关闭侧边栏
        if close_weixin:main_window.close()
        return profile
...
class FriendSettings():
    ...
    @staticmethod
    def clear_chat_history(friend:str,search_pages:int=None,is_maximize:bool=None,close_weixin:bool=None):
        '''该方法用来清空与好友的聊天记录
        Args:
            friend:好友备注
            search_pages:在会话列表中查找好友时滚动列表的次数,默认为5,一次可查询5-12人,为0时,直接从顶部搜索栏搜索好友信息打开聊天界面
        ...
        chatinfo_pane,main_window=Navigator.open_chatinfo(friend=friend,is_maximize=is_maximize,search_pages=search_pages)
        chatinfo_button=main_window.child_window(**Buttons.ChatInfoButton)
        chatinfo_pane.child_window(**Buttons.ClearChatHistoryButton).click_input()
        main_window.child_window(**Buttons.EmptyButton).click_input()
        chatinfo_button.click_input()
        if close_weixin:main_window.close()
    ...
    def change_remark(friend:str,remark:str,description:str=None,phoneNum:str=None,clear_phoneNum:bool=False,search_pages:int=None,is_maximize:bool=None,close_weixin:bool=None):
        ...
        if search_pages is None:
            search_pages=GlobalConfig.search_pages
        profile_pane,main_window=Navigator.open_friend_profile(friend=friend,is_maximize=is_maximize,search_pages=search_pages)
        profile_pane.child_window(**Buttons.MoreButton).click_input()
    ...
    def get_common_groups(friend:str,search_pages:int=None,is_maximize:bool=None,close_weixin:bool=None)->list[str]:
        ...
        scroll_pos=MousePos(profile_window).ProfileWindowScrollPos
        mouse.scroll(coords=scroll_pos,wheel_dist=-100)
        shared_groups_label=profile_window.child_window(**Texts.SharedGroupsText)
        if shared_groups_label.exists(timeout=2):
            shared_groups_button=shared_groups_label.parent().parent().descendants(control_type='Button')[0]
            number_text=shared_groups_button.descendants(control_type='Text')[-1]
            total_num=int(number_text.window_text().replace('个','').replace('個',''))
            shared_groups_button.click_input()
            popup_window=desktop.window(title_re=r'\d+',control_type='Window',class_name='mmui::ProfileUniquePop')
...
class Files():
    ...
    def save_chatfiles(friend:str,number:int,target_folder:str=None,is_maximize:bool=None,close_weixin:bool=None):
        ...
        def find_similar_files(filename, target_dir):
            ...
            for f in os.listdir(target_dir):
                ...
                if file_base.startswith(base_name):
                    result.append(os.path.join(target_dir, f))
            return result
        ...
        def extract_info(text:str):
            ...
            dirname=f'{year}-{month}' if month>=10 else f'{year}-0{month}'
        ...
        if fileList.exists(timeout=0.2):
            ...
            if (not_download not in label) and (expired not in label) and (send_interrupt not in label):
                filename,dirname=extract_info(label)
        ...
        for filename,dirname in zip(filenames,dirnames):
            base_dir=os.path.join(chatfile_dir,dirname)
            filepath=os.path.join(base_dir,filename)#
            #将pyweixin(2).pdf这种带(3)后缀的同名文件也一起导出,只多不少
            similar_files=find_similar_files(filename,base_dir)
            similar_files.append(filepath)
            files.extend(similar_files)
        SystemSettings.copy_files(files,target_folder=target_folder)
    ...
    def export_videos(year:str=time.strftime('%Y'),month:str=time.strftime('%m'),target_folder:str=None)->list[str]:
        ...
        exported_videos=[]
        chatfile_dir=Tools.where_video_folder()
        folders=os.listdir(chatfile_dir)#微信聊天文件所在文件顶部文件夹
        #先找到所有以年份开头的文件夹,并将得到的文件夹名字与其根目录chatfile_folder这个路径join
        filtered_folders=[os.path.join(chatfile_dir,folder) for folder in folders if folder.startswith(year)]
        if month:
        ...
        for folder_path in filtered_folders:#遍历筛选后的每个文件夹
            ...
            videos_in_folder=[os.path.join(folder_path,filename) for filename in  os.listdir(folder_path) if filename.endswith('.mp4')]
            SystemSettings.copy_files(videos_in_folder,target_folder)
        exported_videos=[os.path.join(chatfile_dir,filename) for filename in os.listdir(target_folder) if filename.endswith('.mp4')]
    ...
    def export_wxfiles(year:str=time.strftime('%Y'),month:str=time.strftime('%m'),target_folder:str=None)->list[str]:
        ...
        exported_files=os.listdir(target_folder)#尽管路径全部存在但不一定就可以导出(文件被打开或占用时导出会失败),导出的文件一定是最终opy_files的
...
class Moments():
    ...
    def dump_recent_posts(recent:Literal['Today','Yesterday','Week','Month']='Today',number:int=None,with_name:bool=False,save_detail:bool=False,target_folder:str=None,is_maximize:bool=None,close_weixin:bool=None)->list[dict]:
        ...
        def parse_post(content_listitem:ListItemWrapper):
            ...
            if with_name:
                PostNamePos=MousePos(content_listitem).PostNamePos
    ...
    def like_posts(recent:Literal['Today','Yesterday','Week','Month']='Today',with_name:bool=False,number:int=None,callback:Callable[[str],str]=None,is_maximize:bool=None,close_weixin:bool=None)->list[dict]:
        ...
        def comment(content_listitem:ListItemWrapper,comment_listitem:ListItemWrapper,content:str):
            #评论操作
            EllipsisPos=MousePos(content_listitem).EllipsisPos
    ...
    def dump_friend_posts(friend:str,number:int,save_detail:bool=False,target_folder:str=None,search_pages:int=None,is_maximize:bool=None,close_weixin:bool=None)->list[dict]:
        ...
        def save_media(sns_detail_list:ListViewWrapper,photo_num:int,video_num:int,detail_folder:str,content:str):
            ...
            if photo_num:
                ...
                PostDetailImagePos=MousePos(comment_detail).PostDetailImagePos
                PostDetailImageClickPos=MousePos(sns_detail_list).PostDetailImageClickPos
...
class Messages():
    ...
    def send_audios_to_friend(friend,audios:list[str],audio_length:int=None,clear:bool=None,send_delay:int=None,search_pages:int=None,is_maximize:bool=None,close_weixin:bool=None):
        ...
        if version.parse(current_version)>=version.parse('4.1.9'):
            ...
            if audios:
                ...
                AudioButtonPos=MousePos(send_audio_button).AudioButtonPos
                MainWindowPos=MousePos(main_window).MainWindowPos
                MousePos(send_audio_button).AudioButtonPos
    ...
    def message_chain(group:str,content:str=None,theme:str=None,example:str=None,description:str=None,search_pages:bool=None,
       is_maximize:bool=None,close_weixin:bool=None)->None:
        ...
        if Tools.is_group_chat(main_window):
            ...
            if isinstance(description,str):
                AddContentText=solitaire_window.child_window(**Texts.AddContentText)
                SolitairePos=MousePos(AddContentText).SolitairePos
    ...
    @staticmethod
    def check_new_messages(is_maximize:bool=None,search_pages:int=None,close_weixin:bool=None)->dict[str,list[dict]]:
        '''
        该方法用来检查一遍微信会话列表内的新消息
        Args:
            search_pages:打开好友聊天窗口时在会话列表中查找好友时滚动列表的次数,默认为5,一次可查询5-12人,为0时,直接从顶部搜索栏搜索好友信息打开聊天界面
            is_maximize:微信界面是否全屏，默认不全屏
            close_weixin:任务结束后是否关闭微信，默认关闭
        Returns:
            newMessages:新消息字典,{'好友':pull_messages方法返回值}
        '''
        if is_maximize is None:
            is_maximize=GlobalConfig.is_maximize
        if close_weixin is None:
        ...
        if new_message_num:
            ...
            newMessages=dict(zip(new_message_dict.keys(),new_messages))
    ...
    def dump_sessions(chat_only:bool=False,is_maximize:bool=None,close_weixin:bool=None)->list[tuple]:
        '''
        该方法用来获取会话列表内所有聊天对象的名称,最后聊天时间,以及最后一条聊天消息,使用时建议全屏这样不会有遗漏!
        Args:
            chat_only:只获取会话列表中有消息的好友(ListItem底部有灰色消息不是空白),默认为False
            is_maximize:微信界面是否全屏，默认不全屏
            close_weixin:任务结束后是否关闭微信，默认关闭
    ...
    def pull_messages(friend:str,number:int,is_json:bool=False,myName:str=None,groupMembers:list=[],search_pages:int=None,is_maximize:bool=None,close_weixin:bool=None)->list[dict]:
        ...
        if not edit_area.exists(timeout=0.1):
            ...
            if not chat_list.children(control_type='ListItem'):
                ...
                if is_group_chat:
                    if groupMembers:details_without_name=[]
                    if not groupMembers:details_without_name=traverse_message(main_window,select=False,number=number)
                    contents,senders,message_tpyes=parse_group_messages(details_with_name,details_without_name,groupMembers)
    ...
    @staticmethod
    def dump_chat_history(friend:str,number:int,groupMembers:list=[],search_content:str=None,is_json:bool=False,save_detail:bool=False,
    target_folder:str=None,search_pages:int=None,is_maximize:bool=None,close_weixin:bool=None)->list[dict]:
        ...
        Args:  
            friend:好友名称
            number:获取的消息数量
            groupMembers:friend对应的群成员列表(可提前通过Contacts.get_groupMembers_info导出保存至本地),如果friend是群聊且传入了该参数则直接解析发送人(无需遍历二次)
        ...
        if is_group_chat:
            ...
            contents,senders,timestamps,message_types=parse_group_chat_history(details_with_name,details_without_name,groupMembers)
        chat_history_window.close()
    ...
    @staticmethod
    def search_chat_history(keyword:str,number:int=None,with_content:bool=False,is_maximize:bool=None,close_weixin:bool=None)->dict[str,list[str]]:
        '''
        该方法用来在微信顶部搜索关键字然后遍历查找聊天记录
        Args:
            keyword:聊天记录关键字
            number:获取前number条相关的聊天记录,在数量达到number时结束(不传入该参数默认获取全部)
            with_content:是否获取具体的聊天记录内容,为False时只获取聊天记录数量
            is_maximize:微信界面是否全屏，默认不全屏
            close_weixin:任务结束后是否关闭微信，默认关闭
        Returns:
        ...
        if chat_history_window is not None:
            ...
            while True:
                ...
                if not with_content:
                    chat_history_detail[current_item.window_text()]=int(re.search(r'\d+',total_label.window_text()).group(0))
                ...
                if next_item is None:
                    break
                SearchChatPos=MousePos(next_item).SearchChatPos
    ...
    def save_media(friend:str,number:int,target_folder:str=None,is_maximize:bool=None,close_weixin:bool=None,search_pages:int=None)->None:
        ...
        if target_folder is None:
            ...
            print(f'未传入文件夹路径,聊天图片或视频将保存至 {target_folder}')
        saved_num=0
        presses_num=1 if version.parse(GlobalConfig.Version)>=version.parse('4.1.9') else 2
        ...
        if not image_container.exists(timeout=0.2):#看一下是否存在聊天记录列表，如果不存在说明没有聊天记录    
            chat_history_window.close()
...
class Monitor():
    ...
    def listen_on_chat(dialog_window:WindowSpecification,duration:str,groupMembers:list=[],save_file:bool=False,save_media:bool=False,target_folder:str=None,close_dialog_window:bool=True)->dict:
        ...
        if (save_file or save_media ) and target_folder is None:
            target_folder=os.path.join(os.getcwd(),f'{friend}_listen_on_chat聊天文件保存')
        ...
        if chatList.children(control_type='CheckBox'):
            visibile_items=chatList.children(control_type='CheckBox')
            initial_runtimeids=[item.element_info.runtime_id for item in visibile_items]
        if not chatList.children(control_type='CheckBox'):initial_runtimeids=[]
        end_timestamp=time.time()+duration#根据秒数计算截止时间
        SystemSettings.open_listening_mode(volume=False)
        while time.time()<end_timestamp:
            visibile_items=chatList.children(control_type='CheckBox')
            newItems=[item for item in visibile_items if item.element_info.runtime_id not in initial_runtimeids]
            if newItems: 
                #改为列表轮询
                for item in newItems:
                    texts.append(item.window_text())
                    if item.class_name()=='mmui::ChatItemView' and not timestamp_pattern.search(item.window_text()):
                        memberEvents.append(item.window_text())
                    if item.class_name()=='mmui::ChatBubbleItemView' and link_label in item.window_text():
                        link_count+=1
                    if item.class_name()=='mmui::ChatBubbleReferItemView' and image_labal in item.window_text():
                        image_count+=1
                    if item.class_name()=='mmui::ChatBubbleReferItemView' and video_label in item.window_text():
                        video_count+=1#视频需要下载直接右键复制不行,需要先点击,如果时间长,要等半天，不太方便
                    if item.class_name()=='mmui::ChatBubbleItemView' and file_label in item.window_text():
                        filename=file_pattern.search(item.window_text()).group(1)
                        filepath=os.path.join(chatfile_folder,timestamp,filename)
                        files.append(filepath)
                total+=len(newItems)
                initial_runtimeids=[item.element_info.runtime_id for item in visibile_items]
...
class Settings():
    ...
    def Log_out(is_maximize:bool=None,close_weixin:bool=None):
        ...
        settings_window.child_window(**Buttons.LogOutButton).click_input()
    ...
    def change_style(style:int,is_maximize:bool=None,close_weixin:bool=None):
        ...
        settings_window=Navigator.open_settings(is_maximize=is_maximize,close_weixin=close_weixin)
        settings_window.child_window(**Buttons.GeneralButton).click_input()
    ...
    def change_fontsize(value:int=2,is_maximize:bool=None,close_weixin:bool=None):
        ...
        value_map={1:0,2:24,3:48,4:96,5:120,6:144,7:168,8:192,9:220}
    ...
    def notification_alert(alert_map:dict={'newMessage':True,'Call':True,'Moments':True,'Game':True,'Interaction_only':True},
        ...
        if close_weixin is None:
            close_weixin=GlobalConfig.close_weixin
        settings_window=Navigator.open_settings(is_maximize=is_maximize,close_weixin=close_weixin)
        settings_window.child_window(**Buttons.NotificationButton).click_input()
