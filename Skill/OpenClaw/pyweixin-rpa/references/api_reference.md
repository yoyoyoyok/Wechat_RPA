...
Messages.dump_chat_history(
    ...
    is_json: bool = False,        # 返回值为list[dict]格式的JSON字符串
...
**返回值**: `dict[str, (int|list[str])]` — `{好友备注: (聊天记录数量|[匹配的聊天内容列表].}`,返回值字典中键的类型由采纳数
...
拉取指定好友的聊天消息（只读当前可视区域，不滚动窗口，比 `dump_chat_history` 更轻量）。
