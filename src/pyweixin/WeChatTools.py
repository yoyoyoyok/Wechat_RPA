...
class Tools():
    ...
    def activate_chatList(chatList:ListViewWrapper):
        ...
        '''
        ActivatePos=MousePos(chatList).ActiveChatListPos
    ...
    def get_search_result(friend:str,search_result:ListViewWrapper)->(ListItemWrapper|None):
        ...
        if xtable_label.intersection(texts):#功能比如文件传输助手,微信支付的class_name是mmui::XTableCell
            ...
            #微信4.1.13+搜索结果顶部会出现"搜索网络结果"建议区(同名条目并非功能入口),需优先取分区标签下的条目
            in_xtable_section=False
            for listitem in listitems:
                if listitem.window_text() in xtable_label:
                    in_xtable_section=True
                    continue
                if in_xtable_section and listitem.window_text()==friend:
                    return listitem
            #回退:若分区下没有匹配项,保持旧逻辑取第一个同名条目
            matched=[listitem for listitem in listitems if listitem.window_text()==friend]
            if matched:
                return matched[0]
        return None
            ...
            if selected:
                if selected[0].class_name()!='mmui::ChatItemView':
                    ...
                    if len(runtime_ids)>2 and runtime_ids[-1]==runtime_ids[-2]:
                        break
                    ChatHistorySelectPos=MousePos(selected[0]).ChatHistorySelectPos
...
class Navigator():
    ...
    def open_weixin(is_maximize:bool=None,window_size:tuple=None)->(WindowSpecification|None):
        ...
        # 微信 4.1.15+ may use the current chat/feature name as the top-level
        # title (for example, "遮雨蓬") instead of "微信". Fall back to the
        # visible Qt main window rather than constructing a UIA wrapper for
        # handle 0, which surfaces a misleading COM subscriber error.
        if hwnd==0:
            candidates=[]
            def collect_main_window(candidate, _):
                if win32gui.GetClassName(candidate)=='Qt51514QWindowIcon':
                    left, top, right, bottom=win32gui.GetWindowRect(candidate)
                    visible_bonus=1 if win32gui.IsWindowVisible(candidate) else 0
                    area=max(0, right-left)*max(0, bottom-top)
                    candidates.append((visible_bonus, area, candidate))
            win32gui.EnumWindows(collect_main_window, None)
            if candidates:
                hwnd=max(candidates)[2]
        if hwnd:
            win32gui.ShowWindow(hwnd, win32con.SW_RESTORE)
        if not hwnd:
            raise NotFoundError('未找到微信主窗口，请确认微信已登录且主界面可见')
    ...
    def open_chatinfo(friend:str,is_maximize:bool=None,search_pages:int=None)->tuple[WindowSpecification,WindowSpecification]:
        '''
        该方法用来打开好友或群聊的聊天信息界面,即在聊天界面中点击···后右侧弹出的界面
        Args:
            friend:好友或群聊名称
            is_maximize:微信界面是否全屏,默认不全屏
        Returns:
        ...
        if not chatinfo_button.exists(timeout=0.2):
            ...
            if not chatinfo_pane.exists(timeout=0.2):
    ...
    def open_my_profile(is_maximize:bool=None)->tuple[WindowSpecification,WindowSpecification]:
        ...
        coords=MousePos(weixin_button).AvatarPos
    ...
    def open_settings(is_maximize:bool=None,close_weixin:bool=None)->WindowSpecification:
        ...
        if close_weixin is None:
            close_weixin=GlobalConfig.close_weixin
        main_window=Navigator.open_weixin(is_maximize=is_maximize)
        main_window.child_window(**SideBar.More).click_input()
        main_window.child_window(**Buttons.SettingsButton).click_input()
    ...
    def open_contacts(is_maximize:bool=None)->tuple[ListViewWrapper,WindowSpecification]:
        ...
        if is_maximize is None:
            is_maximize=GlobalConfig.is_maximize
        main_window=Navigator.open_weixin(is_maximize=is_maximize)
        main_window.child_window(**SideBar.Contacts).click_input()
    ...
    def open_chatfiles(is_maximize:bool=None,window_maximize:bool=None,close_weixin:bool=None)->WindowSpecification:
        ...
        if window_maximize is None:
            window_maximize=GlobalConfig.window_maximize
        main_window=Navigator.open_weixin(is_maximize=is_maximize)
        main_window.child_window(**SideBar.More).click_input()#点击更多按钮
    ...
    def search_friend(friend:str,is_maximize:bool=None)->WindowSpecification:
        ...
        #先看看当前聊天界面是不是好友的聊天界面
        current_chat_label=Texts.CurrentChatNameText#主界面顶部的好友名称
        current_chat_label['title']=friend
        current_chat=main_window.child_window(**current_chat_label)
    ...
    def open_seperate_dialog_window(friend:str,select:bool=False,is_maximize:bool=None,window_minimize:bool=False,close_weixin:bool=None)->WindowSpecification:
        ...
        def get_search_result(friend:str,search_result:ListViewWrapper)->(ListItemWrapper|None):
            ...
            if xtable_label.intersection(texts):#功能比如文件传输助手,微信支付的class_name是mmui::XTableCell
                listitems=search_result.children(control_type='ListItem',class_name="mmui::XTableCell")
                #微信4.1.13+搜索结果顶部会出现"搜索网络结果"建议区(同名条目并非功能入口),需优先取"功能"分区下的条目
                in_xtable_section=False
                for listitem in listitems:
                    if listitem.window_text() in xtable_label:
                        in_xtable_section=True
                ...
                #回退:若"功能"分区下没有匹配项,保持旧逻辑取第一个同名条目
                matched=[listitem for listitem in listitems if listitem.window_text()==friend]
                if matched:
    ...
    def open_chat_history(friend:str,TabItem:Literal['文件','图片与视频','链接','音乐与音频','小程序','视频号','日期','群成员']=None,
        search_pages:int=None,is_maximize:bool=None,close_weixin:bool=None)->WindowSpecification:
        '''
        该方法用于打开好友聊天记录界面
        Args:
