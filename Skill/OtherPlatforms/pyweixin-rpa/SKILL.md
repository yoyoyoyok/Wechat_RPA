...
name: pyweixin-rpa
description: >-
  基于pywinauto实现的Windows微信UI自动化工具，支持消息收发、文件传输、
  朋友圈操作和通讯录管理。当用户提及微信、WeChat、pyweixin、发消息、
  朋友圈或Windows微信RPA时使用。
  ⚠️平台限制：本工具仅支持Windows系统。若当前环境非Windows，
  请立即中止操作并告知用户。
---
...
**用外部调度器触发：** 系统计划任务、CI 平台的 cron 表达式，或 AI 助手的定时能力在指定时间执行该脚本。
...
- 调度器（无论是计划任务、CI 还是 AI 助手）已经具备成熟的定时能力，无需在脚本中重复造轮子。
- 纯干活脚本 = 可测试、可复用、可手动执行。
